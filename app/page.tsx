...
export default function BusinessCycleDashboard() {
  const sectors = [
    {
      ...
      rules: ['Permits and starts both negative YoY', 'Residential score above 1.0', 'If home sales also weaken, early warning strengthens'],
    },
    {
      id: 'durable',
      name: 'Durable Goods',
      ...
      rules: ['ISM below 50 raises caution', 'Two of three demand indicators weakening lifts score', 'Broad consumer softness confirms durable weakness'],
    },
    {
      id: 'transportation',
      name: 'Transportation',
      ...
      rules: ['Orders plus freight both negative indicates capex stress', 'Truck weakness adds cyclical confirmation', 'Three-way deterioration lifts transportation above 1.25'],
    },
    {
      id: 'labor',
      name: 'Labor',
      ...
      rules: ['Claims rising alone is caution, not confirmation', 'Unemployment plus payroll slowing signals broad deterioration', 'Labor score over 1.0 raises recession probability'],
    },
  ];
  ...
  const warningOn = sectors.find((s) => s.id === 'residential')?.score >= 1 && sectors.find((s) => s.id === 'durable')?.score >= 1;
  const confirmationOn = sectors.find((s) => s.id === 'labor')?.score >= 1;

  function cyclePhase(score) {
  ...
  function Sparkline({ values, yAxis, xAxis, tall = false, longLookback = false, scaleLabel = 'Indicator scale' }) {
    const width = 360;
    ...
    const points = values
      .map((v, i) => {
        const x = leftPad + (i / (values.length - 1)) * innerWidth;
    ...
    return (
      <svg viewBox={`0 0 ${width} ${height}`} className={tall ? 'h-48 w-full' : 'h-28 w-full'}>
        <text x={leftPad} y={10} fontSize="10" className="fill-slate-500">{scaleLabel}</text>
        ...
        {yAxis.map((label, idx) => {
          const y = topPad + (idx / (yAxis.length - 1)) * innerHeight;
          return (
            <g key={`y-${label}`}>
              <text x={4} y={y + 4} fontSize="10" className="fill-slate-500">{label}</text>
              <line x1={leftPad} y1={y} x2={width - rightPad} y2={y} stroke="currentColor" className="text-slate-200" strokeDasharray="3 3" />
            </g>
          );
        })}

        {xAxis.map((label, idx) => {
          const x = leftPad + (idx / (xAxis.length - 1)) * innerWidth;
          return (
            <text key={`x-${label}`} x={x - 12} y={height - 6} fontSize="10" className="fill-slate-500">{label}</text>
          );
        })}

        <polyline fill="none" stroke="currentColor" strokeWidth="3" points={points} className="text-slate-700" />
        {longLookback ? <text x={width - 118} y={10} fontSize="10" className="fill-slate-500">Lookback: 2000 to present</text> : null}
      </svg>
    );
  }
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div>
              <div className="text-sm font-medium uppercase tracking-[0.18em] text-slate-500">Business Cycle Dashboard</div>
              <h1 className="mt-2 text-3xl font-semibold text-slate-900">EPB-style cycle engine</h1>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:w-[420px]">
              <div className="rounded-2xl bg-slate-50 px-5 py-4 ring-1 ring-slate-200">
                ...
                <div className={`mt-1 text-sm font-medium ${phaseTone(cyclePhase(weightedScore))}`}>{cyclePhase(weightedScore)}</div>
              </div>
              <div className="rounded-2xl bg-slate-50 px-5 py-4 ring-1 ring-slate-200">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            ...
            <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
              ...
              <div className="mt-3 space-y-2 text-sm">
                <div className={`rounded-xl px-3 py-2 ${warningOn ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700'}`}>Early warning: {warningOn ? 'On' : 'Off'}</div>
                <div className={`rounded-xl px-3 py-2 ${confirmationOn ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>Labor confirmation: {confirmationOn ? 'On' : 'Off'}</div>
              </div>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-slate-500">{sector.weight}% weight · {sector.weightLabel}</div>
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between text-sm">
                  ...
                  <span className="text-slate-600">{trendIcon(sector.trendDirection)} {trendLabel(sector.trendDirection)}</span>
                </div>
                ...
                <div className="mt-4 rounded-2xl bg-white/80 p-3 ring-1 ring-white/60">
                  ...
                  <div className="relative mt-2 h-6">
                    <div
                      className={`absolute top-0 h-6 w-1 rounded-full ${sector.score < 0.75 ? 'bg-green-600' : sector.score < 1.5 ? 'bg-amber-600' : 'bg-red-600'}`}
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                ...
                <div className="mt-1 text-xl font-semibold text-slate-900">{trendIcon(selected.trendDirection)} {trendLabel(selected.trendDirection)}</div>
              </div>
              <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
              ...
              {selected.indicators.map((indicator) => (
                <div key={indicator.name} className="rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-200">
                  ...
                  <div className="mt-4 rounded-2xl bg-white p-3 ring-1 ring-slate-200">
                    <Sparkline values={indicator.longTrend} yAxis={indicator.yAxis} xAxis={indicator.longXAxis} tall longLookback scaleLabel={indicator.scaleLabel} />
                  </div>
                  <div className="mt-3 rounded-2xl bg-white p-3 ring-1 ring-slate-200">
                    ...
                    <Sparkline values={indicator.rocTrend} yAxis={indicator.rocAxis} xAxis={indicator.xAxis} scaleLabel="Rate-of-change scale" />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
              ...
              <div className="mt-4 space-y-3 text-sm text-slate-600">
                <div className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200">Weight housing more heavily than later-cycle sectors in the composite score.</div>
                <div className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200">Track diffusion so you can see how much weakness is broadening across the dashboard.</div>
                <div className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200">Show level charts and separate rate-of-change charts because turning points often appear in momentum first.</div>
                <div className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200">Keep the main screen focused on sequence: Residential → Durable → Transportation → Labor.</div>
              </div>
            </div>
          </div>
        </div>
